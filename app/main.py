...
UNSTRUCTURED_PLATFORM_URL_DEFAULT = "https://platform.unstructuredapp.io/"
...
def _resolve_format_fusion_table_target(
    ...
    schema_hint, table_hint = _split_object_name_hint(object_hint)
    ...
    if not table_hint:
        table_hint = f"{vector_store_name}_ff_chunks"

    table_name = _sanitize_teradata_identifier(table_hint, fallback="fusion_chunks")
...
def _teradata_table_exists(qualified_table_sql: str) -> bool:
    if execute_sql is None:
        raise RuntimeError("teradataml.execute_sql is unavailable.")
    try:
        execute_sql(f"SELECT TOP 1 1 FROM {qualified_table_sql}")
        return True
...
def _ensure_unstructured_teradata_table(schema_name: str | None, table_name: str, clear_rows: bool = True) -> list[str]:
    if execute_sql is None:
        raise RuntimeError("teradataml.execute_sql is unavailable.")

    warnings: list[str] = []
    qualified_table = _qualified_table_sql(schema_name, table_name)
    table_exists = _teradata_table_exists(qualified_table)
...
"""
    ...
    if clear_rows:
        try:
            execute_sql(f"DELETE FROM {qualified_table}")
        except Exception as ex:
            warnings.append(f"Failed to clear destination table before run: {ex}")
    return warnings


def _count_teradata_rows(schema_name: str | None, table_name: str) -> int | None:
    if execute_sql is None:
        return None
    qualified_table = _qualified_table_sql(schema_name, table_name)
    try:
        cursor = execute_sql(f"SELECT COUNT(*) FROM {qualified_table}")
    except Exception:
        return None
...
def _workflow_file_inputs(document_files: list[str]):
...
def _new_unstructured_client():
    api_key = str(os.getenv("UNSTRUCTURED_API_KEY") or os.getenv("UNSTRUCTURED_API_KEY_AUTH") or "").strip()
    if not api_key:
        raise RuntimeError("UNSTRUCTURED_API_KEY is not set.")

    platform_url = (
        str(os.getenv("UNSTRUCTURED_PLATFORM_URL", UNSTRUCTURED_PLATFORM_URL_DEFAULT)).strip()
        or UNSTRUCTURED_PLATFORM_URL_DEFAULT
    )
    from unstructured_client import UnstructuredClient

    return UnstructuredClient(api_key_auth=api_key, server_url=f"{platform_url.rstrip('/')}/")
...
def _run_unstructured_workflow(client, workflow_id: str, document_files: list[str]) -> str:
    from unstructured_client.models import operations, shared

    run_inputs = _workflow_file_inputs(document_files)
...
def _wait_for_unstructured_job(client, job_id: str, timeout_seconds: int, poll_interval_seconds: int) -> str:
    ...
    while True:
        ...
        info = getattr(resp, "job_information", None)
        current_status = str(getattr(info, "status", last_status)).upper()
...
def _apply_format_fusion_pipeline(
    exec_payload: dict,
    create_values: dict[str, str],
    vector_store_name: str,
    ...
    try:
        ...
        job_id = _run_unstructured_workflow(client, workflow_id=workflow_id, document_files=document_files)
    ...
    row_count = _count_teradata_rows(schema_name=effective_schema_name, table_name=table_name)
...
async def upload_and_prepare_create(request: Request):
    ...
    if doc_pipeline_mode == "format_fusion":
        try:
            exec_payload, format_fusion_summary = _apply_format_fusion_pipeline(
